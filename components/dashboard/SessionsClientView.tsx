...
interface SessionsClientViewProps {
  initialSessions: SessionItem[];
  courses: Course[];
  totalStudents: number;
}

export default function SessionsClientView({
  initialSessions,
  courses,
  totalStudents,
}: SessionsClientViewProps) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-16">
        {[
          { label: "Active Sessions", value: `${initialSessions.length} Scheduled`, icon: <Video className="w-6 h-6" />, color: "text-z-red", bg: "bg-z-red/10" },
          { label: "Total Students", value: totalStudents.toString(), icon: <Users className="w-6 h-6" />, color: "text-emerald-500", bg: "bg-emerald-500/10" },
          { label: "System Alerts", value: "Fully operational", icon: <Bell className="w-6 h-6" />, color: "text-blue-500", bg: "bg-blue-500/10" },
        ].map((stat, i) => (
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 gap-6">
          {filteredSessions.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-20 glass-card rounded-[3.5rem] border-2 border-dashed border-zinc-200/50 min-h-[200px] text-center">
              <Video className="w-12 h-12 text-zinc-300 mb-4" />
              <h3 className="text-xl font-black text-zinc-700">No live sessions scheduled</h3>
              <p className="text-xs text-zinc-500 font-bold mt-2">
                Click the button below or in the header to schedule your first stream.
              </p>
            </div>
          ) : (
              ...
              <div key={session.id} className="glass-card p-8 rounded-[2.5rem] group hover:scale-[1.01] transition-all duration-500 premium-shadow border border-white/40 flex items-center justify-between">
                ...
                <div className="flex items-center space-x-12">
                   ...
                   <div className="flex items-center space-x-4">
                     <Link href={`/classroom/${session.videoUrl}`}>
          ...
          <div 
            onClick={() => setIsModalOpen(true)}
            className="glass-card p-8 rounded-[2.5rem] border-2 border-dashed border-zinc-200 flex items-center justify-center group hover:border-z-red/30 hover:bg-z-red/[0.02] transition-all cursor-pointer"
          >
            <div className="flex items-center space-x-4">
